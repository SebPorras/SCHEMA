...
def main():
    ...
    pdb_contacts = generate_contacts(args)

    return 0

def generate_contacts(args):
...
def find_pdb_key(args) -> None | str:
...
def setup_parser():
    ...
    # optional arguments
    # contact args
    parser.add_argument("-pdbal", type = validate_file, action="store", help = "(Optional) In ALN format. If this argument is not provided, then the PDB file's ID (e.g., 1G68) will be extracted, and the sequence having that ID in the multiple sequence alignment file will be used")
    parser.add_argument("-chains", action="store", help = "The PDB chain identifers (e.g. -chain A B) Chains 'A' and ' ' are included by default.")
    parser.add_argument("-cout", action="store", help = "(Optional) Will create or overwrite a contact file (eg contacts.txt)")
    ...
    parser.add_argument("-o", action = "store", help = "Will create or overwrite a file called averages.txt")
