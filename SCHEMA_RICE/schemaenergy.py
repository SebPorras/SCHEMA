...
def confirm_arguments(arg_dict):
	...
	try:
		...
		if not ARG_MULTIPLE_SEQUENCE_ALIGNMENT_FILE in arg_keys:
			print(
				"  You must provide a library file (-%s <file>)"
				% ARG_MULTIPLE_SEQUENCE_ALIGNMENT_FILE
			)
			res = False
		...
		if not ARG_CROSSOVER_FILE in arg_keys:
			print(
				"  You must provide a crossover file (-%s <file>)" % ARG_CROSSOVER_FILE
			)
			res = False
		...
		if not (ARG_PRINT_E in arg_dict or ARG_PRINT_M in arg_dict):
			print(
				"  No output specified; use -E to print disruption and/or -m to print mutation"
			)
			res = False
...
def main(args):
	...
	with open(arg_dict[ARG_CROSSOVER_FILE], "r") as file:
		crossovers = schema.readCrossoverFile(file)
