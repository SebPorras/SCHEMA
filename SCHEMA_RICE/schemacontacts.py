...
import sys, os, pdb_reader, schema
...
def confirm_arguments(arg_dict):
	...
	try:
		...
		if not ARG_MULTIPLE_SEQUENCE_ALIGNMENT_FILE in arg_keys:
			print(
				"  You must provide an alignment file (-%s <file>)"
				% ARG_MULTIPLE_SEQUENCE_ALIGNMENT_FILE
			)
			res = False
			...
			print(
				...
				% arg_dict[ARG_MULTIPLE_SEQUENCE_ALIGNMENT_FILE]
			)
			res = False
...
def main(args):
	...
	if ARG_PDB_ALIGNMENT_FILE in arg_dict:
		if not os.path.isfile(arg_dict[ARG_PDB_ALIGNMENT_FILE]):
			print(
				"  Can't find PDB/parent alignment file %s"
				% arg_dict[ARG_PDB_ALIGNMENT_FILE]
			)
			return
		else:
	...
	with open(msa_file, "r") as msa_f:
		parent_list = schema.readMultipleSequenceAlignmentFile(msa_f)
	...
	with open(pdb_file, "r") as file:
		residues = pdb_reader.File().read(file)
	...
	if (
	...
	):  # Just get PDB sequence from the multiple sequence alignment
		try:
		...
		with open(parent_pdb_alignment_file, "r") as par_pdb_file:
	...
	contact_distance = (
		4.5  # Residues closer than this distance, in angstroms, are in contact.
	)
	...
	if ARG_OUTPUT_FILE in arg_dict:
		with open(arg_dict[ARG_OUTPUT_FILE], "w") as contact_file:
