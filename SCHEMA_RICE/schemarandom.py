...
import sys, os, random, time, schema, raspp
...
def confirm_arguments(arg_dict):
	...
	try:
		...
		if not ARG_MULTIPLE_SEQUENCE_ALIGNMENT_FILE in arg_keys:
			print(
				"  You must provide a library file (-%s <alignment file>)"
				% ARG_MULTIPLE_SEQUENCE_ALIGNMENT_FILE
			)
			res = False
		...
		if not ARG_CONTACT_FILE in arg_keys:
			print(
				"  You must provide a contact file (-%s <contact file>)"
				% ARG_CONTACT_FILE
			)
			res = False
		...
		if not ARG_NUM_CROSSOVERS in arg_keys:
			print(
				"  You must specify the number of crossovers (-%s <number of crossovers>)"
				% ARG_NUM_CROSSOVERS
			)
			res = False
...
def main(args):
	...
	for libnum in range(num_libraries):
		crossovers = schema.generateRandomCrossovers(
			len(new_parents[0]), num_fragments - 1, min_length
		)
	...
	for crossovers in random_crossovers:
		...
		filtered_contacts = schema.getSCHEMAContactsWithCrossovers(
			pdb_contacts, parents, crossovers
		)
		all_chimeras = []
		...
		for chim_index in range(max_chimeras):
			...
			E = schema.getChimeraDisruption(
				chimera_blocks, filtered_contacts, fragments, parents
			)
